...
function codePointConvert(char, upperStart, lowerStart, digitStart, upperExceptions, lowerExceptions) {
    const upper = UPPERCASE.indexOf(char);
    const lower = LOWERCASE.indexOf(char);
    const digit = char >= '0' && char <= '9' ? Number(char) : -1;
    if (upper >= 0)
        return upperExceptions?.[char] ?? String.fromCodePoint(upperStart + upper);
    if (lower >= 0)
        return lowerExceptions?.[char] ?? String.fromCodePoint(lowerStart + lower);
    if (digit >= 0 && digitStart !== undefined)
        return String.fromCodePoint(digitStart + digit);
    return char;
}
export function convertFont(text, font) {
    if (font.combining) {
        if (font.passthrough) {
    ...
    return [...text].map(char => {
        ...
        if (font.upperMap || font.lowerMap) {
            ...
            if (uIdx >= 0 && font.upperMap)
                return [...font.upperMap][uIdx] ?? char;
            if (lIdx >= 0 && font.lowerMap)
                return [...font.lowerMap][lIdx] ?? char;
            if (dIdx >= 0 && font.digitMap)
                return [...font.digitMap][dIdx] ?? char;
            return char;
        }
        ...
        if (font.upper !== undefined && font.lower !== undefined) {
            return codePointConvert(char, font.upper, font.lower, font.digits, font.upperExceptions, font.lowerExceptions);
        }
        return char;
    }).join('');
}
